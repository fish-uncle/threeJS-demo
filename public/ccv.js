...
var ccv = {
  ...
  detect_objects: parallable("https://cdn.shenzhepei.com/face-demo/ccv.js", function (canvas, cascade, interval, min_neighbors) {
...
onmessage = function (event) {
  const data = (typeof event.data == "string" && event.data !== '') ? JSON.parse(event.data) : event.data;
  if (data) {
    const scope = {"shared": data.shared};
    if (data.name) {
      const result = parallable.core[data.name].apply(scope, [data.input, data.id, data.worker]);
      try {
        postMessage(result);
      } catch (e) {
